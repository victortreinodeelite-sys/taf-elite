...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (user) {
      const { data: student } = await supabase.from('students').select('anamnese_done').eq('user_id', user.id).single()
      router.push(student?.anamnese_done ? '/dashboard' : '/cadastro')
    }
  }
    ...
    <div className='min-h-screen flex items-center justify-center px-4 bg-[#070707]'
      ...
      <div className='relative w-full max-w-md'>
        <div className='text-center mb-8'>
          ...
          <p className='text-[#555] text-xs tracking-[.3em] uppercase mt-2'>Plataforma de Preparacao Tatica</p>
        </div>
        <div className='bg-[#0f0f0f] border border-[#1c1c1c] rounded-2xl p-8'
          style={{ boxShadow: '0 0 50px rgba(225,6,0,.07)' }}>
          <h1 className='text-white font-bold text-xl mb-1'>Acesso Operacional</h1>
          <p className='text-[#555] text-sm mb-6'>Entre com suas credenciais para continuar.</p>
          <form onSubmit={handleLogin} className='space-y-4'>
            ...
            <button type='submit' disabled={loading}
              ...
              {loading ? 'ACESSANDO...' : 'INICIAR MISSAO'}
            </button>
          </form>
          <div className='mt-6 pt-6 border-t border-[#181818] text-center'>
            <p className='text-[#484848] text-sm'>Nao tem acesso?{' '}
              <Link href='/cadastro' className='text-[#E10600] hover:text-[#ff3333] transition-colors font-semibold'>Criar conta</Link>
            </p>
          </div>
        </div>
      </div>
